...
impl BwPolicy {
    ...
    // ) -> InnerResult<u64> {
    //     let sql = r#"
    //     UPDATE bw_policy SET name = COALESCE($1, name), settings =
    // COALESCE($2, settings)     WHERE policy_id = $3 AND account_id = $4
    ...
    // and account_id = $1"#;     let map =
    // sqlx::query_scalar(sql).bind(account_id);     Ok(map.fetch_one(db).
    // await?) }
    //
